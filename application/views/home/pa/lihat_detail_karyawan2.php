            ...
            <?php 
                if($header[0]->status == 'PROCESS'){
                    ...
                    <div class="alert alert-warning alert-dismissible text-center" role="alert" style="position:relative;z-index:501;width:100%">
                        PA ini di Re-Assesment oleh <?php echo $sebutan.' '.$bio[0]->nama?>.
                    </div> 
            <?php }elseif($header[0]->status == 'RE-ASSESMENT'){
                    ...
                    <div class="alert alert-danger alert-dismissible text-center" role="alert" style="position:relative;z-index:501;width:100%">
                        PA telah Re-Assesment oleh <?php echo $sebutan .' '.$bio[0]->nama?>, klik <a href="<?php echo base_url('pa/lihatDetailKaryawan2/'.$header[0]->ref_kdpa);?>"><u><?php echo $header[0]->ref_kdpa?></u></a> untuk melihat PA yang berjalan. 
                    </div>
            <?php }?>
            ...
            <div class="container-fluid rounded shadow" style="background: #f2f2f2;position: relative; height: 82vh;overflow-y:scroll" id="konten">
                <div class="row pt-3 pl-3 pr-3">
                    <div class="col-md-3 text-left">
                        <a href="<?php echo base_url('pa/lihatAllKaryawan2');?>" class="btn btn-sm btn-secondary" onclick="loader()"><i class="fas fa-chevron-left"></i> Kembali</a>&ensp;
                    ...
                    <div class="col-md-3 text-right">
                        ...
                        <?php if($header[0]->status == 'ACCEPTED'){?>
                        <a href="<?php echo base_url('pa/updateApproved/'.$kode_pa)?>" class="btn btn-sm btn-success" onclick="return confirm('Pastikan Detail Rekomendasi & Keterangan PA sudah dibaca terlebih dahulu sebelum melakukan APPROVED.\n\nPilih CANCEL untuk membaca kembali detail Rekomendasi & Keterangan.\n\nInfo lebih lanjut hubungi IT.')"><i class="fas fa-check"></i> APPROVED</a>
                        <a href="<?php echo base_url('pa/updateAssesment/'.$kode_pa)?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin Ingin Re-assesment Data ini?')">RE-ASSESMENT</a>
                        <?php }

                        if($header[0]->user_input == $this->session->userdata('nik') AND $header[0]->status == 'PROCESS'){?>
                        <a href="<?php echo base_url('pa/updateApproved/'.$kode_pa)?>" class="btn btn-sm btn-success" onclick="return confirm('Pastikan Detail Rekomendasi & Keterangan PA sudah disesuaikan dengan penilaian PA yang telah di Re-Assesment. \n\n Yakin Ingin melanjutkan?')"><i class="fas fa-check"></i> APPROVED</a>
                        <?php } ?>
                    </div>
                </div>
                ...
                <div class="table-responsive pl-4 pr-3 pb-4">
                    ...
                    <div class="col-md-12 text-right pt-3">
                        <a href="javascript:;" data-toggle="modal" class="btn btn-sm btn-tosca" data-placement="bottom" title="REVISI ALL" data-target="#revisi-all">REVISI ALL</a>
                    </div>
                    <?php }?>
                    <div class="col-md-12 pt-3">
                        ...
                        <thead>
                            <tr>
                                ...
                                <?php if($header[0]->user_input != $this->session->userdata('nik')){?>
                        ...
                        <tbody>
                           ...
                           <tr>
                               ...
                               <td style="border: 1px solid #000;">
                                   ...
                                   <a href="javascript:;" class="btn btn-xs btn-danger" data-toggle="modal" data-placement="bottom" title="KETERANGAN" data-target="#ket_atasan<?php echo $detail->kode_fk?>">K</a>
                               </td>
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <div class="col-md-12" align="justify">
                            ...
                            <input type="hidden" name="dept" value="2">
...
<!-- END MODAL EDIT-->
                            ...
                            <?php } 
                            if($header[0]->user_input != $this->session->userdata('nik')){?>
...
<div class="modal fade" id="rekomendasi-keterangan" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <?php if($header[0]->status =='ACCEPTED'){?>
            <form method="POST" action="<?php echo base_url('pa/revisiRK');?>"> 
            <?php }elseif($header[0]->user_input == $this->session->userdata('nik')){?>
            ...
            <div class="modal-body">
                <div class="form-group row" style="font-size:20px">
                    ...
                    <div class="col-md-8">
                        ...
                        <textarea cols="10" rows="5" class="form-control-sm form-control" name="rekomendasi" <?php if($header[0]->user_input != $this->session->userdata('nik')){ echo 'readonly'; }?>><?php echo $header[0]->rekomendasi?></textarea>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <?php }?>
                        ...
                        <textarea cols="10" rows="5" class="form-control-sm form-control" name="keterangan" <?php if($header[0]->user_input != $this->session->userdata('nik')){ echo 'readonly'; }?>><?php echo $header[0]->keterangan?></textarea>
                    </div>
                </div>
            </div>
            <?php if($header[0]->status =='ACCEPTED'){?>
            <div class="modal-footer">
                <!-- <input type="submit" class="btn btn-sm btn-danger" value="REVISI RK" name="submit" onclick="return confirm('Yakin Ingin Revisi Rekomendasi & Keterangan Data?')"> -->
            </div>
            <?php }elseif($header[0]->user_input == $this->session->userdata('nik')){?>
            <div class="modal-footer">
                <input type="submit" class="btn btn-sm btn-success" value="SIMPAN" onclick="return confirm('Yakin Ingin Simpan Data?')">
...
<!-- Modal Revisi-->
<div class="modal fade" id="revisi-all" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-nobby">
                <h5 class="modal-title text-light" id="exampleModalLabel">KONFIRMASI REVISI</h5>
                <button type="button" class="close text-light" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <?php
            ?>
            <form method="POST" action="<?php echo base_url('pa/revisiAll');?>"> 
            <div class="modal-body">
                <div class="form-group row">
                    <div class="col-md-12">
                        <input type="hidden" name="kode_pa" value="<?php echo $kode_pa?>">
                        <label><font color="red">Bila dilakukan REVISI ALL maka status PA akan mulai dari awal kembali agar bisa direvisi oleh user yang bersangkutan dan atasannya.<br><br> Silakan klik tombol <strong>REVISI ALL</strong> untuk melanjutkan.</font></label>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <input type="submit" class="btn btn-sm btn-success" value="REVISI ALL" onclick="return confirm('Yakin Ingin Revisi All Data?')">
            </div>
            </form>
        </div>
    </div>
