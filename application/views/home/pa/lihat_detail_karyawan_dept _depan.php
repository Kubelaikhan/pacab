        ...
        <div id="content" class="content text-center" style="">
            ...
            <?php }
                    ...
                    <div class="alert alert-danger alert-dismissible text-center" role="alert" style="position:relative;z-index:501;width:100%">
                        PA ini Re-Assesment oleh <?php echo $sebutan.' '.$bio[0]->nama?>, klik <a href="<?php echo base_url('pa/lihatDetailBawahanDept/'.$header[0]->ref_kdpa);?>"><u><?php echo $header[0]->ref_kdpa?></u></a> untuk lanjut ke tahap penilaian berikutnya.
                    </div>
            <?php }  ?>
            <div class="container-fluid rounded shadow" style="background: #fcfbfc;position: relative; height: 82vh;overflow-y:scroll" id="konten">
                <div class="row pt-3 pl-3 pr-3">
                    <div class="col-md-3 text-left">
                        <!-- <a href="javascript: history.go(-2)" class="btn btn-sm btn-secondary" onclick="loader()"><i class="fas fa-chevron-left"></i> Kembali</a>&ensp; -->
                        <a href="<?php echo base_url('pa/lampiranPA/'.$kode_pa)?>" target="_blank" class="btn btn-sm btn-tosca">LAMPIRAN</a>
                    </div>
                    <div class="col-md-5"><h4>FORMULIR PERFORMANCE APPRAISAL</h4></div>
                    <div class="col-md-4 text-right">
                        ...
                        <?php if($header[0]->user_input == $this->session->userdata('nik') AND $header[0]->status == 'PROCESS'){
                            if ( $bod_dept > 0) {?>
                        ...
                        if($header[0]->user_input == $header[0]->nik AND $header[0]->status == 'PROCESS' AND $bod_dept > 0){?>
                            <a href="<?php echo base_url('pa/updateApproved/'.$kode_pa)?>" class="btn btn-sm btn-success" onclick="return confirm('Pastikan Detail Rekomendasi & Keterangan PA sudah dibaca terlebih dahulu sebelum melakukan APPROVED.\n\nPilih CANCEL untuk membaca kembali detail Rekomendasi & Keterangan.\n\nInfo lebih lanjut hubungi IT.')"><i class="fas fa-check"></i> APPROVED</a>
                        <?php }

                        $user_update_bod = $this->pa_model->cek_bod_dept($header[0]->user_update);
                        if($header[0]->status == 'AGREE' AND $header[0]->user_input != $this->session->userdata('nik')){
                            if($bod_dept == 0){?>
                                <a href="<?php echo base_url('pa/updateConfirmedDept/'.$kode_pa)?>" class="btn btn-sm btn-success" onclick="return confirm('Pastikan sudah mengupload File KPI di tombol Lampiran.\n\nYakin Ingin Confirmed  Data?')"><i class="fas fa-check"></i> CONFIRM SOH</a>
                        <?php }elseif($bod_dept == 1){?>
                                <a href="<?php echo base_url('pa/updateApproved/'.$kode_pa)?>" class="btn btn-sm btn-success" onclick="return confirm('Pastikan Detail Rekomendasi & Keterangan PA sudah dibaca terlebih dahulu sebelum melakukan APPROVED.\n\nPilih CANCEL untuk membaca kembali detail Rekomendasi & Keterangan.\n\nInfo lebih lanjut hubungi IT.')"><i class="fas fa-check"></i> APPROVED</a>
                        <?php }?>
                                <a href="<?php echo base_url('pa/updateAssesment/'.$kode_pa)?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin Ingin Re-assesment Data ini?')">RE-ASSESMENT</a>
                        <?php }elseif($header[0]->status == 'RE-ASSESMENT' AND $header[0]->user_update == $manager){?>
                                <a href="<?php echo base_url('pa/lihatDetailBawahanDept/'.$header[0]->ref_kdpa);?>"  class="btn btn-sm" style="background:#b3cccc"><font style="font-size:12px; color: #527a7a;">Re-Assesment SOH</font></a>
                        <?php }elseif($header[0]->status == 'RE-ASSESMENT' AND $user_update_bod > 0){?>
                                <a href="<?php echo base_url('pa/lihatDetailBawahanDept/'.$header[0]->ref_kdpa);?>"  class="btn btn-sm" style="background:#b3cccc"><font style="font-size:12px; color: #527a7a;">Re-Assesment BOD</font></a>
                        <?php }?>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content" style="width:100%">
                                        ...
                                        <form method="POST" action="<?php echo base_url('pa/updateKomentarNilai');?>"> 
                                            ...
                                            <?php if($header[0]->status == 'PROCESS' AND $header[0]->user_input == $this->session->userdata('nik')){?>
    ...
    <div class="modal fade" id="rekomendasi-keterangan" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <?php if($header[0]->user_input == $this->session->userdata('nik') || $this->session->userdata('nik') == '10000001' || $this->session->userdata('nik') == '10000002'){ 
